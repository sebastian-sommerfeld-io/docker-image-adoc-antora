...
== What is `{image-name}`?
The `{image-name}` image is a utility Docker image used to build the HTML contents from Asciidoc sources for link:https://www.sommerfeld.io/docs/projects[www.sommerfeld.io] (see link:https://github.com/sebastian-sommerfeld-io/website-sommerfeld-io[`sebastian-sommerfeld-io/website-sommerfeld-io` on Github] for more information about this website).
